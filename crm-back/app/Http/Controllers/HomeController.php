...
foreach ($assigneduser as $single) {
    // Get the count of pending leads for the user
    $pendingLeads = DB::table('leads')
        ->join('tbl_employees', 'leads.assign_id', '=', 'tbl_employees.id')
        ->where('leads.assign_id', $single->id)
        ->where(function ($query) {
            $query->where('leads.sale_status', 0)
                ->orWhereNull('leads.sale_status');
        })
        ->count();

    // Add the username and pending leads count to the array
    $usersWithLeads[] = [
        'username' => $single->name, // Assuming $single has the 'username' property
    ...
    return view('dashboard', compact('role', 'usersWithLeads','assigneduser'));
}
